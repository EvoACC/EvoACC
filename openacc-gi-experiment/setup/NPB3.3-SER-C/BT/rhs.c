	...
	for (k = 1; k <= grid_points[2]-2; k++) {
		...
		for (j = 1; j <= grid_points[1]-2; j++) {
			...
			for (i = 1; i <= grid_points[0]-2; i++) {
				...
				rhs[k][j][i][1] = rhs[k][j][i][1] + dx2tx1 *
				                  ...
				                  tx2 * (u[k][j][i+1][1]*up1 - u[k][j][i-1][1]*um1 +(u[k][j][i+1][4]- square[k][j][i+1]-u[k][j][i-1][4]+ square[k][j][i-1])*c2);
				...
				rhs[k][j][i][4] = rhs[k][j][i][4] + dx5tx1 *
				                  ...
				                  xxcon5 * (u[k][j][i+1][4]*rho_i[k][j][i+1] - 2.0*u[k][j][i][4]*rho_i[k][j][i] +u[k][j][i-1][4]*rho_i[k][j][i-1]) -
				                  tx2 * ( (c1*u[k][j][i+1][4] - c2*square[k][j][i+1])*up1 -(c1*u[k][j][i-1][4] - c2*square[k][j][i-1])*um1 );
		...
		for (j = 1; j <= grid_points[1]-2; j++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  (-4.0*u[k][j][i-1][m] + 6.0*u[k][j][i][m] -4.0*u[k][j][i+1][m] + u[k][j][i+2][m]);
		...
		for (j = 1; j <= grid_points[1]-2; j++) {
			...
			for (i = 3; i <= grid_points[0]-4; i++) {
				...
				for (m = 0; m < 5; m++) {
					...
					rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
					                  (  u[k][j][i-2][m] - 4.0*u[k][j][i-1][m] + 6.0*u[k][j][i][m] - 4.0*u[k][j][i+1][m] + u[k][j][i+2][m] );
		...
		for (j = 1; j <= grid_points[1]-2; j++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  ( u[k][j][i-2][m] - 4.0*u[k][j][i-1][m] + 6.0*u[k][j][i][m] - 4.0*u[k][j][i+1][m] );
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  ( u[k][j][i-2][m] - 4.*u[k][j][i-1][m] +5.*u[k][j][i][m] );
	...
	for (k = 1; k <= grid_points[2]-2; k++) {
		...
		for (j = 1; j <= grid_points[1]-2; j++) {
			...
			for (i = 1; i <= grid_points[0]-2; i++) {
				...
				rhs[k][j][i][2] = rhs[k][j][i][2] + dy3ty1 *
				                  ...
				                  ty2 * (u[k][j+1][i][2]*vp1 - u[k][j-1][i][2]*vm1 +(u[k][j+1][i][4] - square[k][j+1][i] - u[k][j-1][i][4] + square[k][j-1][i])*c2);
				...
				rhs[k][j][i][4] = rhs[k][j][i][4] + dy5ty1 *
				                  ...
				                  yycon5 * (u[k][j+1][i][4]*rho_i[k][j+1][i] - 2.0*u[k][j][i][4]*rho_i[k][j][i] +u[k][j-1][i][4]*rho_i[k][j-1][i]) -
				                  ty2 * ((c1*u[k][j+1][i][4] - c2*square[k][j+1][i]) * vp1 -(c1*u[k][j-1][i][4] - c2*square[k][j-1][i]) * vm1);
		...
		for (i = 1; i <= grid_points[0]-2; i++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  (-4.0*u[k][j-1][i][m] + 6.0*u[k][j][i][m] -4.0*u[k][j+1][i][m] + u[k][j+2][i][m]);
		...
		for (j = 3; j <= grid_points[1]-4; j++) {
			...
			for (i = 1; i <= grid_points[0]-2; i++) {
				...
				for (m = 0; m < 5; m++) {
					...
					rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
					                  (  u[k][j-2][i][m] - 4.0*u[k][j-1][i][m] + 6.0*u[k][j][i][m] - 4.0*u[k][j+1][i][m] + u[k][j+2][i][m] );
		...
		for (i = 1; i <= grid_points[0]-2; i++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  ( u[k][j-2][i][m] - 4.0*u[k][j-1][i][m] + 6.0*u[k][j][i][m] - 4.0*u[k][j+1][i][m] );
		...
		for (i = 1; i <= grid_points[0]-2; i++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  ( u[k][j-2][i][m] - 4.*u[k][j-1][i][m] +5.*u[k][j][i][m] );
		...
		for (j = 1; j <= grid_points[1]-2; j++) {
			...
			for (i = 1; i <= grid_points[0]-2; i++) {
				...
				rhs[k][j][i][3] = rhs[k][j][i][3] + dz4tz1 *
				                  ...
				                  tz2 * (u[k+1][j][i][3]*wp1 - u[k-1][j][i][3]*wm1 +(u[k+1][j][i][4] - square[k+1][j][i] - u[k-1][j][i][4] + square[k-1][j][i])*c2);
				...
				rhs[k][j][i][4] = rhs[k][j][i][4] + dz5tz1 *
				                  ...
				                  zzcon5 * (u[k+1][j][i][4]*rho_i[k+1][j][i] - 2.0*u[k][j][i][4]*rho_i[k][j][i] +u[k-1][j][i][4]*rho_i[k-1][j][i]) -
				                  tz2 * ( (c1*u[k+1][j][i][4] - c2*square[k+1][j][i])*wp1 -(c1*u[k-1][j][i][4] - c2*square[k-1][j][i])*wm1);
	...
	for (j = 1; j <= grid_points[1]-2; j++) {
		...
		for (i = 1; i <= grid_points[0]-2; i++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  (-4.0*u[k-1][j][i][m] + 6.0*u[k][j][i][m] -4.0*u[k+1][j][i][m] + u[k+2][j][i][m]);
		...
		for (j = 1; j <= grid_points[1]-2; j++) {
			...
			for (i = 1; i <= grid_points[0]-2; i++) {
				...
				for (m = 0; m < 5; m++) {
					...
					rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
					                  (  u[k-2][j][i][m] - 4.0*u[k-1][j][i][m] + 6.0*u[k][j][i][m] - 4.0*u[k+1][j][i][m] + u[k+2][j][i][m] );
	...
	for (j = 1; j <= grid_points[1]-2; j++) {
		...
		for (i = 1; i <= grid_points[0]-2; i++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  ( u[k-2][j][i][m] - 4.0*u[k-1][j][i][m] + 6.0*u[k][j][i][m] - 4.0*u[k+1][j][i][m] );
	...
	for (j = 1; j <= grid_points[1]-2; j++) {
		...
		for (i = 1; i <= grid_points[0]-2; i++) {
			...
			for (m = 0; m < 5; m++) {
				...
				rhs[k][j][i][m] = rhs[k][j][i][m] - dssp *
				                  ( u[k-2][j][i][m] - 4.*u[k-1][j][i][m] +5.*u[k][j][i][m] );
